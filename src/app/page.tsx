...
export default function HomePage() {
  const { packages = [], searchQuery = '', selectedSeason = 'all' } = useTravelData();

  // Memoize filtered results to maximize layout performance
  ...
  }, [packages, searchQuery, selectedSeason]);

  // Memoize dashboard metric counts
  const stats = useMemo(() => {
    return {
      total: packages.length,
      international: packages.filter((pkg) => pkg.category === 'international').length,
      lowInventory: packages.filter((pkg) => pkg.inventory?.minSeatsLeft <= 3).length,
    };
  }, [packages]);

  return (
    <div className="w-full bg-slate-50 min-h-screen">
      {/* Hero Showcase Portals */}
      <section className="relative isolate overflow-hidden bg-slate-950 pt-36 text-white w-full">
        <img
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-6 pb-12 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:pb-16 w-full">
          <div className="max-w-3xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href="#collections"
                className="rounded-lg bg-amber-500 px-6 py-3 text-xs font-black uppercase tracking-widest text-slate-950 shadow-lg transition duration-200 hover:bg-amber-400"
              >
                Explore collections
              </a>
              <a
                href="tel:18002093344"
                className="rounded-lg border border-white/20 bg-white/10 px-6 py-3 text-xs font-bold uppercase tracking-widest text-white backdrop-blur transition duration-200 hover:bg-white/15"
              >
                Call concierge
              </a>
            </div>
          ...
          <div className="grid gap-3 rounded-xl border border-white/15 bg-white/10 p-4 text-sm backdrop-blur-md sm:grid-cols-3 lg:grid-cols-1">
            <div className="rounded-lg bg-slate-950/55 p-4 transition-all hover:bg-slate-950/75">
            ...
            <div className="rounded-lg bg-slate-950/55 p-4 transition-all hover:bg-slate-950/75">
              ...
              <p className="text-2xl font-black text-white">{stats.international}</p>
            ...
            <div className="rounded-lg bg-slate-950/55 p-4 transition-all hover:bg-slate-950/75">
              ...
              <p className="text-2xl font-black text-white">{stats.lowInventory}</p>
      ...
      {/* Main Filter Section */}
      <section id="collections" className="mx-auto max-w-7xl px-6 py-12 sm:px-8 lg:py-16 w-full">
        ...
        <div className="mb-10 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 w-full">
          <SearchBar />
          <TabSwitcher />
        </div>
        ...
        {filteredPackages.length > 0 ? (
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3 w-full">
            {filteredPackages.map((pkg) => (
              <Card key={pkg.id || pkg.title} pkg={pkg} />
            ))}
          </div>
        ) : (
          <div className="mx-auto max-w-xl rounded-xl border border-dashed border-slate-200 bg-white px-6 py-16 text-center shadow-sm">
